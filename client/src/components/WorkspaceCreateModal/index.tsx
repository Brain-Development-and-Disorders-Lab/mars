...
// Custom types
import { ResponseMessage, WorkspaceCreateProps, WorkspaceModel } from "@types";
...
const WorkspaceCreateModal = (props: WorkspaceCreateProps) => {
  ...
  const handleCreateWorkspaceClick = async () => {
    ...
    if (result.data?.createWorkspace.success) {
      ...
      if (
        ...
        props.setWorkspaces(workspaces.data.workspaces);
        if (created) {
          props.setWorkspaceIdentifier(created._id);
        }
  ...
  return (
    <Modal
      ...
      size={"full"}
      closeOnEsc={props.workspaceIdentifier !== ""}
    >
      <ModalOverlay />
      <ModalContent>
        <ModalHeader p={"2"}>
          ...
          <Flex gap={"2"} direction={"column"}>
            {props.workspaceIdentifier === "" ? (
              ...
              <Text fontSize={"sm"} fontWeight={"semibold"} color={"gray.400"}>
                ...
                well as inviting collaborators to work together on experiments.
              </Text>
            )}
            <Text fontSize={"sm"} fontWeight={"semibold"} color={"gray.400"}>
