...
async function bootstrapUserData(userId: string) {
  ...
  if (entities && entities[0] && entities[1]) {
    ...
    Entities.addProduct(
      { name: entity1.name, _id: entity1._id },
      { name: entity2.name, _id: entity2._id },
    );
    Entities.addOrigin(
      { name: entity2.name, _id: entity2._id },
      { name: entity1.name, _id: entity1._id },
