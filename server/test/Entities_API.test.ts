...
describe('checkEntitiesOwnership', () => {
    ...
    beforeEach(() => {
        ...
        (EntitiesModule.Entities.getOne as jest.Mock).mockResolvedValue({ _id: 'entity-id', owner: 'user-id' } as never);
    ...
    describe('checkEntitiesOwnership Middleware', () => {
      ...
      it('should respond with 404 if the entity is not found', async () => {
          ...
          (EntitiesModule.Entities.getOne as jest.Mock).mockResolvedValue(null as never);
      ...
      it('should respond with 403 if user is not the owner of the entity', async () => {
          ...
          expect(res.json).toHaveBeenCalledWith({ message: 'User does not have permission to access this entity' });
          expect(next).not.toHaveBeenCalled();
      });
