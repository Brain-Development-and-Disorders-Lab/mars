...
describe("Attributes model", () => {
  ...
  it("should get an Attribute", async () => {
    ...
    const result: ResponseData<string> = await Attributes.create({
      name: "TestAttribute",
      owner: "henry.burgess@wustl.edu",
      archived: false,
      description: "Attribute description",
    ...
    // Get the Attribute
    const attribute: AttributeModel | null = await Attributes.getOne(identifier);
  ...
  it("should return multiple requested Attribute entries", async () => {
    ...
    const identifiers: string[] = result.slice(0, NUM_ATTRIBUTES / 2).map((attribute) => attribute._id);
    ...
    expect(resultMany.map((attribute) => attribute._id)).toEqual(expect.arrayContaining(identifiers));
  });
